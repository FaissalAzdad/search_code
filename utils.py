...
class SortedListCheapestCost(Queue):
    def __init__(self):
        self.A = []
        self.start = 0
...
class SortedListWithHeuristic(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        auxSum = self.auxSum + 1
        self.sortByUsingHeuristic()

    def sortByUsingHeuristic(self):
        self.A.sort(key = lambda x : (x.path_cost + int(self.problem.h(x))))
